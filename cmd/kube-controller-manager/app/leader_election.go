...
func (m *leaderElectionManager) StartedLeading(ctx context.Context, electionID string) bool {
	return m.enqueue(ctx, electionID, true)
}

func (m *leaderElectionManager) StoppedLeading(electionID string) {
	m.enqueue(context.Background(), electionID, false)
}
...
func (m *leaderElectionManager) Error(err error) {
	if err == nil {
		return
	}
